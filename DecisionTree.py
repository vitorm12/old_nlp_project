...
class DecisionTree:
    def __init__(self,read):
        ...
        self.negation = None
        if read:
            bt = open("classifiers/dt-Negation.pkl", 'rb')
            self.negation = load(bt)
            bt.close()

            sen = open("classifiers/dt-sentiwordnet.pkl", 'rb')
            self.sentiTrainer = load(sen)
            sen.close()

            bw = open("classifiers/dt-binary-words.pkl", 'rb')
            self.binaryTrainer = load(bw)
            bw.close()

            raw = open("classifiers/dt-raw-count.pkl", 'rb')
            self.raw_countTrainer = load(raw)
            raw.close()
    ...
    def create_binary_trainer(self):
        print("Creating Decision Tree classifier in classifiers/dt-binary-words.jbl")
    ...
    def create_senti(self):
        print("Creating Decision Tree classifier in classifiers/dt-sentiwordnet.jbl")
        start_time = time.time()
    ...
    def negation_create(self):
        print("Creating DecisionTree classifier in classifiers/dt-Negation.jbl")
        start_time = time.time()
    ...
    def dump_to_file(self):
        output = open("classifiers/dt-Negation.pkl", 'wb')
        dump(self.negation, output, -1)
        output.close()

        output = open("classifiers/dt-sentiwordnet.pkl", 'wb')
        dump(self.sentiTrainer, output, -1)
        output.close()

        output = open("classifiers/dt-binary-words.pkl", 'wb')
        dump(self.binaryTrainer, output, -1)
        output.close()

        output = open("classifiers/dt-raw-count.pkl", 'wb')
